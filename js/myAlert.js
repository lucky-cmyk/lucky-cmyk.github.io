function removeModa() {
    $('.myModa').fadeOut(300);
    setTimeout(function() {
        $('.myModa').remove()
    }, 300)
}
function myToast(message) {
    var top = window.screen.height * 0.4;
    $('body').append('<div class="myToast">' + message + '</div>');
    var top = (window.screen.height - $('.myToast').height()) / 2;
    var left = ($('body').width() - $('.myToast').width()) / 2;
    $('.myToast').css({
        'top': top + 'px',
        'left': left + 'px',
        'textAlign': 'center'
    });
    setTimeout(function() {
        $('.myToast').fadeOut(300);
        setTimeout(function() {
            $('.myToast').remove()
        }, 300)
    }, 2000);
}
function myConfirm(options) {
    var option = {
        title: "Hint",
        message: "The programmer was stupid and forgot to enter the prompt……",
        callback: function() {}
    };
    if (typeof (options) == "string") {
        option.message = options
    } else {
        option = $.extend(option, options)
    }
    var top = window.screen.height * 0.4;
    $('body').append('<div class="myModa"><div class="myAlertBox" style="margin-top:' + top + 'px"><h6>' + option.title + '</h6><p>' + option.message + '</p><div class="col2 clearfix"><div class="col first" style="margin-right: 20px;"><div class="ConfrimBtn exit">cancel</div></div><div class="col"><div class="ConfrimBtn Confrimsure">confirm</div></div></div></div></div>');
    //  $('body').css('overflow-y', 'hidden');
    $('.ConfrimBtn.exit').click(function() {
        removeModa();
        $('body').css('overflow-y', 'scroll')
    });
    $('.ConfrimBtn.Confrimsure').click(function() {
        removeModa();
        $('body').css('overflow-y', 'scroll');
        option.callback()
    })
}
function myAlert(options) {
    var option = {
        title: "Hint",
        message: "The programmer was stupid and forgot to enter the prompt……",
        callback: function() {}
    };
    if (typeof (options) == "string") {
        option.message = options
    } else {
        option = $.extend(option, options)
    }
    var top = window.screen.height * 0.4;
    $('body').append('<div class="myModa"><div class="myAlertBox"  style="margin-top:' + top + 'px"><h6>' + option.title + '</h6><p>' + option.message + '</p><div class="AlertBtn Alertsure">confirm</div></div></div>');
    //  $('body').css('overflow-y', 'hidden');
    $('.AlertBtn.Alertsure').click(function() {
        removeModa();
        $('body').css('overflow-y', 'scroll');
        option.callback();
    })
}
function myLoadding(message) {
    var m = message || 'Loadding...';
    var top = window.screen.height * 0.4;
    $('body').append('<div class="myModa"><div class="myLoadding"><img src="data:image/gif;base64,"/><p>' + m + '</p></div></div>');
    var top = (window.screen.height - $('.myLoadding').height()) / 2;
    var left = ($('body').width() - $('.myLoadding').width()) / 2;
    $('.myLoadding').css({
        'top': top + 'px',
        'left': left + 'px'
    })
}
